...
use Theme_Name_Name_Space\Inc\Config\{
	Meta_Box1_Config, Meta_Box2_Config
};
...
class Meta_box {
	...
	public function render_content( $post ) {
		if ( $this->id == 'meta_box_1_id' ) {
			$this->render_meta_box1( $post, $this->meta_key, $this->single, $this->action, $this->nonce_name );
		}
		if ( $this->id == 'meta_box_2_id' ) {
			$this->render_meta_box2( $post, $this->meta_key, $this->single, $this->action, $this->nonce_name );
		}
	}
	...
	public function save( $post_id ) {
		...
		if ( $this->id == 'meta_box_1_id' ) {
			$this->save_meta_box1( $post_id, $_POST, $this->meta_key );
		}
		if ( $this->id == 'meta_box_2_id' ) {
			$this->save_meta_box2( $post_id, $_POST, $this->meta_key );
		}
